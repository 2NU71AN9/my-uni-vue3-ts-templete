...
export const useDeviceStore = defineStore('device', {
	...
	getters: {
		safeAreaInset: (state) => ({
			top: Math.max(state.deviceInfo.safeArea.top, state.deviceInfo.statusBarHeight),
		...
		orientation: (state) => (state.deviceInfo.screenWidth > state.deviceInfo.screenHeight ? 'landscape' : 'portrait'),
		isPortrait(): boolean {
